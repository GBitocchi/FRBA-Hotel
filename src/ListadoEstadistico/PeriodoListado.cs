        ...
        private void btnAceptar_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                switch (listado)
                {
                    ...
                    case 4:
                        {
                            DataTable clientes = DataBase.realizarConsulta("select top 5 H.hues_mail as 'Mail', H.hues_documento as 'Documento', hues_nombre as 'Nombre', hues_apellido as 'Apellido' from CAIA_UNLIMITED.Factura F join CAIA_UNLIMITED.Item_Factura I on (F.fact_nro = I.fact_nro) join CAIA_UNLIMITED.Huesped H on (F.hues_documento = H.hues_documento and F.hues_mail = H.hues_mail) join CAIA_UNLIMITED.Estadia E on (E.esta_codigo = F.esta_codigo) join CAIA_UNLIMITED.Reserva R on (R.rese_codigo = E.rese_codigo) join CAIA_UNLIMITED.Regimen W on (W.regi_codigo = R.regi_codigo) where DATEPART(QUARTER, fact_fecha) = " + trimestre + " and YEAR(fact_fecha) = " + anio + " group by H.hues_mail, H.hues_documento, hues_nombre, hues_apellido order by sum(((I.item_monto*I.item_cantidad)/10) + ((DATEDIFF(day, E.esta_fecha_inicio, E.esta_fecha_fin)*W.regi_precio_base)/20)) desc").Tables[0]; 
